...
import type { VendorReceiptInput } from './vendorReceipt';
...
export function vendorBillRowToThermalReceiptData(b: VendorBillRow): ThermalReceiptData {
  ...
  return {
    ...
    items,
    discount: 0,
    serviceFee: Number(b.convenience_fee ?? 0),
    total: Number(b.total ?? 0),
    footer: 'Thank you!',
    userEmail:
      b.user_email != null && String(b.user_email).trim() !== ''
        ? String(b.user_email).trim()
        : undefined,
  };
}

export function vendorReceiptInputToThermalReceiptData(
