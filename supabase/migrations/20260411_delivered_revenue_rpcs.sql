-- Delivery-day revenue: join deduped vendor_bills to delivered orders, bucket by IST calendar date.
-- Dedupe matches get_revenue_by_date (same token + total → keep latest created_at).
...
AS $$
  ...
  joined AS (
    ...
    WHERE o.status = 'delivered'
      ...
      AND (p_from IS NULL OR o.delivery_confirmed_at >= p_from)
      AND (p_to IS NULL OR o.delivery_confirmed_at <= p_to)
  )
  SELECT
    (j.delivery_confirmed_at AT TIME ZONE p_tz)::date AS delivery_date,
