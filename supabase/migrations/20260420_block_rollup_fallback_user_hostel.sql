-- Block × date dashboard: when a saved bill has no hostel block text, roll up using the
-- customer's profile (users.hostel_block) so rows leave "No block" when the profile is set.
...
COMMENT ON FUNCTION get_delivered_revenue_by_block_and_date(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) IS
  'Delivered-day revenue by rollup block key; bill customer_hostel_block first, else users.hostel_block.';
