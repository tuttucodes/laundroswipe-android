-- Single normalization for block rollups (aligned with lib/hostel-block.ts).
-- Fixes missing A / D1 / D2 buckets when bills use "Mens A", "D 1", stacked BLOCK/HOSTEL prefixes, etc.

CREATE OR REPLACE FUNCTION normalize_hostel_block_key(p_raw TEXT)
RETURNS TEXT
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  u := upper(btrim(translate(p_raw, chr(160), ' ')));
  u := regexp_replace(u, '\s+', ' ', 'g');
  u := btrim(regexp_replace(u, '^((?:HOSTEL|H\.|BLK|BLOCK|TOWER|WING)\s*[-/:]?\s*)+', '', 'i'));

  IF u = '' OR u IS NULL THEN
    RETURN 'No block';
  END IF;

  IF u ~ '^BLOCK\s*[-:]?\s*A([^A-Za-z]|$)' THEN RETURN 'A'; END IF;
  IF u ~ '^BLOCK\s*[-:]?\s*D\s*[- ]?\s*1([^0-9]|$)' THEN RETURN 'D1'; END IF;
  IF u ~ '^BLOCK\s*[-:]?\s*D\s*[- ]?\s*2([^0-9]|$)' THEN RETURN 'D2'; END IF;
  IF u ~ '^BLOCK\s*[-:]?\s*D1([^0-9]|$)' THEN RETURN 'D1'; END IF;
  IF u ~ '^BLOCK\s*[-:]?\s*D2([^0-9]|$)' THEN RETURN 'D2'; END IF;

  IF u ~ '^D\s*[- ]?\s*1([^0-9]|$)' THEN RETURN 'D1'; END IF;
  IF u ~ '^D\s*[- ]?\s*2([^0-9]|$)' THEN RETURN 'D2'; END IF;

  IF u ~ '^A([^A-Za-z]|$)' THEN RETURN 'A'; END IF;

  IF u ~ '(^|[^A-Z0-9])A\s*$' THEN RETURN 'A'; END IF;
  IF u ~ '(^|[^A-Z0-9])D\s*[- ]?\s*1\s*$' THEN RETURN 'D1'; END IF;
  IF u ~ '(^|[^A-Z0-9])D\s*[- ]?\s*2\s*$' THEN RETURN 'D2'; END IF;

  RETURN u;
END;
$$;

COMMENT ON FUNCTION normalize_hostel_block_key(TEXT) IS
  'Maps hostel block free text to rollup keys A, D1, D2, No block, or uppercase literal; matches app lib/hostel-block.ts.';
...
AS $$
  ...
  SELECT
    ...
    normalize_hostel_block_key(j.customer_hostel_block) AS block_key,
